...
module.exports.fetchBook = async function (id, action, referer, timeout) {
    let book = []
    let response = await syncFetch("https://mouban.lesslsmore.com/guest/user_book?action=" + action + "&id=" + id, referer, timeout)
    if (response === "OFFLINE") {
    ...
    for (let i in bookResult.comment) {
        ...
        book.push({
            ...
            alt: "https://book.neodb.com/subject/" + comment.item.neodb_id + "/",
...
module.exports.fetchGame = async function (id, action, referer, timeout) {
    let game = []
    let response = await syncFetch("https://mouban.lesslsmore.com/guest/user_game?action=" + action + "&id=" + id, referer, timeout)
    if (response === "OFFLINE") {
        log.warn("Cannot connect to server")
        return game
    }
    ...
    for (let i in gameResult.comment) {
        ...
        game.push({
            title: comment.item.title,
            alt: "https://www.neodb.com/game/" + comment.item.neodb_id + "/",
            image: comment.item.thumbnail,
            meta: metas.join(" / "),
            rating: rating.join(" / "),
            comment: comment.comment
...
module.exports.fetchSong = async function (id, action, referer, timeout) {
    let song = []
    let response = await syncFetch("https://mouban.lesslsmore.com/guest/user_song?action=" + action + "&id=" + id, referer, timeout)
    if (response === "OFFLINE") {
        log.warn("Cannot connect to server")
        return song
    }
    ...
    for (let i in songResult.comment) {
        ...
        song.push({
            title: comment.item.title,
            alt: "https://music.neodb.com/subject/" + comment.item.neodb_id + "/",
            image: comment.item.thumbnail,
            meta: metas.join(" / "),
            rating: rating.join(" / "),
            comment: comment.comment
...
module.exports.fetchMovie = async function (id, action, referer, timeout) {
    let movie = []
    let response = await syncFetch("https://mouban.lesslsmore.com/guest/user_movie?action=" + action + "&id=" + id, referer, timeout)
    if (response === "OFFLINE") {
        log.warn("Cannot connect to server")
        return movie
    }
    ...
    for (let i in movieResult.comment) {
        ...
        movie.push({
            title: comment.item.title,
            alt: "https://movie.neodb.com/subject/" + comment.item.neodb_id + "/",
            image: comment.item.thumbnail,
            meta: metas.join(" / "),
            rating: rating.join(" / "),
            comment: comment.comment
...
module.exports.fetchData = async function (id, referer, type, timeout) {
    let userResult = await syncFetch("https://mouban.lesslsmore.com/guest/check_user?id=" + id, referer, timeout)
